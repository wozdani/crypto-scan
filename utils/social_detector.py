...
def get_social_mentions(symbol):
    """
    Pobiera dane społecznościowe dla danego symbolu z LunarCrush API
    """
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Błąd HTTP {response.status_code} dla social {symbol}")
            return None
        ...
        # Możemy użyć różnych metryk: social_score, tweet_mentions, reddit_score
...
def detect_social_spike(symbol):
    """
    Wykrywa nagły wzrost aktywności społecznościowej dla danego symbolu
    """
...
def get_social_stats(symbol):
    """
    Zwraca szczegółowe statystyki społecznościowe dla danego symbolu
    """
    try:
        api_key = os.getenv('LUNARCRUSH_API_KEY')
        if not api_key:
        ...
        print(f"❌ Błąd pobierania social stats dla {symbol}: {e}")
        return None